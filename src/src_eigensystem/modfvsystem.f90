...
! fill the lower triangle too
    ...
    Subroutine solvewithlapack(system,nstfv,evecfv,evalfv)
        ...
        !local
        Integer :: is, ia, i, m, np, info ,nmatp, j
        Real (8) :: vl, vu
        Real (8) :: ts0, ts1
         ! allocatable arrays
...
!        print*, 'Form a Cholesky factorization of B.'
!!LAPACK-ori
!!        CALL zpptrf( uplo, n, bp, info )
!        CALL zpptrf( 'U', nmatp, system%overlap%zap, info )
!
!!Transform problem to standard eigenvalue problem.
!        print*,'Transform problem to standard eigenvalue problem.'
!!LAPACK-ori
!!        CALL zhpgst( itype, uplo, n, ap, bp, info )
!        CALL zhpgst( 1, 'U', nmatp, system%hamilton%zap, system%overlap%zap, info )
!
!!Solve standard eigenvalue problem.
!       print*,'Solve standard eigenvalue problem.'
!!LAPACK-ori
!!       CALL zhpevx( jobz, range, uplo, n, ap, vl, vu, il, iu, abstol, m, &
!!                     w, z, ldz, work, rwork, iwork, ifail, info )
!
!        CALL zhpevx( 'V', 'I', 'U', nmatp, system%hamilton%zap, vl, vu, 1, nstfv, &
!                     input%groundstate%solver%evaltol, m, &
!                     w, evecfv, nmatmax, work, rwork, iwork, ifail, info )
!
!        evalfv (1:nstfv) = w (1:nstfv)
!
!!Backtransform eigenvectors to the original problem.
!        print*,'Backtransform eigenvectors to the original problem.'
!!LAPACK-ori CALL ztpsv( uplo, trans, 'Non-unit', n, bp, z( 1, j ), 1)
!        do j =1, m
!        CALL ztpsv( 'U', 'N', 'Non-unit', nmatp, system%overlap%zap, evecfv( 1, j ), 1)
!        End do
!
!!used in fleur
!!        CALL ztptrs( 'U', 'N', 'N', nmatp, nstfv, system%hamilton%zap, evecfv, nmatmax, info)
...
!xpx        !Call zhpgvx (1, 'V', 'I', 'U', nmatp, system%hamilton%zap, &
!xpx        !system%overlap%zap, vl, vu, 1, nstfv, &
!xpx        !input%groundstate%solver%evaltol, m, w, evecfv, nmatmax, work, &
!xpx        !rwork, iwork, ifail, info)
!xpx        call ZHEGVX(1, 'V', 'I', 'U', nmatp, system%hamilton%za, nmatp, system%overlap%za, nmatp,&
!xpx         vl, vu, 1, nstfv, input%groundstate%solver%evaltol, &
!xpx         m, w, evecfv, nmatmax, work, 2*nmatp, rwork, iwork, ifail, info )
!xpx        evalfv (1:nstfv) = w (1:nstfv)
!xpx        !
!xpx        !
!xpx        !
!xpx        If (info .Ne. 0) Then
!xpx            Write (*,*)
!xpx            Write (*, '("Error(seceqnfv): diagonalisation failed")')
!xpx            Write (*, '(" ZHPGVX returned INFO = ", I8)') info
!xpx            If (info .Gt. nmatp) Then
!xpx                i = info - nmatp
!xpx                Write (*, '(" The leading minor of the overlap matrix of or&
!xpx           &der ", I8)'                ) i
!xpx                Write (*, '("  is not positive definite")')
!xpx                Write (*, '(" Order of overlap matrix : ", I8)') nmatp
!xpx                Write (*,*)
!xpx            End If
!xpx            Stop
!xpx        End If
...
!xpx>>>>>>>> begin
!     Form a Cholesky factorization of Overlap

        CALL zpotrf( 'U', nmatp, system%overlap%za, nmatp, info )
!LAPACK-ori
!       CALL zpotrf( uplo, n, b, ldb, info ) 

!     Transform problem to standard eigenvalue problem and solve.

        CALL zhegst( 1, 'U', nmatp, system%hamilton%za, nmatp, system%overlap%za, nmatp, info)
!LAPACK-ori 
!       CALL zhegst( itype, uplo, n, a, lda, b, ldb, info )

!chase replace begin here
!chase        CALL zheevx( 'V', 'I', 'U', nmatp, system%hamilton%za, nmatp, vl, vu, 1, nstfv, &
!chase                     input%groundstate%solver%evaltol, m,  &
!chase                     w, evecfv, nmatmax, work, 2*nmatp, rwork, iwork, ifail, info )
!chase!LAPACK-ori
!chase!              CALL zheevx( jobz, range, uplo, n, a, lda, vl, vu, il, iu, abstol,
!chase!       $             m, w, z, ldz, work, lwork, rwork, iwork, ifail,
!chase!       $             info )
!chase
!chase        evalfv (1:nstfv) = w (1:nstfv)
!chase replace end here

!chase>>>>>>>>begin
        nev = nstfv
        nex = max (30, nstfv/4)

        ALLOCATE (herm(nmatp,nmatp),vc(nmatp,nev+nex))
        ALLOCATE (wc(nmatp,nev+nex),etemp(nev+nex))

        DO j=1,nmatp
          DO i=1,j
             herm(i, j) = system%hamilton%za(i, j)
             herm(j, i) = conjg(herm(i,j))
          ENDDO
        ENDDO

        print*, 'before chase'
        CALL c_chase(herm,nmatp,vc,wc,etemp,nev,nex,10,input%groundstate%solver%evaltol,'R','S')

        print*, 'after chase'
        evalfv(1:nev)=etemp(1:nev)

        evecfv(:nmatp,:nev)=vc(:,:nev)    !nsize<=nbasfcn, nev=iu<=neigd

        DEALLOCATE (herm,vc,wc,etemp)
!chase>>>>>>>>end
