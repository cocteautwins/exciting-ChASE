...
!      Real (8) :: magmt_k (lmmaxvr, nrmtmax, natmtot, ndmag)
       ...
       Real (8), Allocatable :: rhoir_k (:)
       Real (8), Allocatable :: magir_k (:, :)
       Real (8), Allocatable :: rhomt_k (:, :, :)
       Real (8), Allocatable :: magmt_k (:, :, :, :)

      Call timesec (ts0)

      Allocate (rhoir_k (ngrtot))
      Allocate (rhomt_k (lmmaxvr, nrmtmax, natmtot))

      If (associated(input%groundstate%spin)) Then
      Allocate (magir_k (ngrtot, ndmag))
      Allocate (magmt_k (lmmaxvr, nrmtmax, natmtot, ndmag))
      End If
...
!      rhoir (:) = rhoir (:) + rhoir_k (:)
      ...
      rhomt (:, :, :) = rhomt (:, :, :) + rhomt_k (:, :, :)
      Deallocate (rhomt_k, rhoir_k)
      If (associated(input%groundstate%spin)) Then
         magmt (:, :, :, :) = magmt (:, :, :, :) + magmt_k (:, :, :, :)
!        magir (:, :) = magir (:, :) + magir_k (:, :)
      Deallocate (magmt_k, magir_k)
      End If
